...
class Tray_CheckoutApi_Block_Info_Standard extends Mage_Payment_Block_Info
{
    ...
    public function getLinkPayment($order) 
    {
        if ($this->getRequest()->getRouteName() != 'checkout') {
            ...
            $splitYapay = $_order->getPayment()->getData('traycheckout_split_number');
            $splitValueYapay = $_order->getPayment()->getData('traycheckout_split_value');
            $paymentMethodYapay = $_order->getPayment()->getData('traycheckout_payment_method_name');
            ...
            if ($method == "traycheckoutapi" && ($_order->getStatus() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT )){
                return '
                    <span>Para efetuar o pagamento, <a href="' . Mage::getBaseUrl() . 'checkoutapi/standard/paymentbackend/order/' . $hash . '">clique aqui</a>.</span><br><br>
                    <span><strong>Parcelas</strong>: ' . $splitYapay .' x R$ ' . $splitValueYapay . ' </span><br>
                    <span><strong>Meio de Pagamento</strong>: ' . $paymentMethodYapay .' </span><br><br>
                    ';
            }

            if ($method == "traycheckoutapi") {
                echo('<span><strong>Parcelas</strong>: ' . $splitYapay .' x R$ ' . $splitValueYapay . ' </span><br> ');
                if ($paymentMethodYapay != NULL) {
                    echo('<span><strong>Meio de Pagamento</strong>: ' . $paymentMethodYapay .' </span><br><br> ');
                }
