    ...
    public function getLinkPayment($order) 
    {
        if ($this->getRequest()->getRouteName() != 'checkout') {
            ...
            if ($method == "traycheckoutapi_bankslip" && (($_order->getStatus() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT ) || ($_order->getStatus() == 'pending' ))){
                return '<p><strong>Linha Digitável: </strong> '.$_order->getPayment()->getData('traycheckout_typeful_line').'</p><span><button onclick="window.open(\''.$_order->getPayment()->getData('traycheckout_url_payment').'\');" class="button btn-proceed-checkout btn-checkout" title="Imprimir Boleto    " type="button"><span><span>Imprimir Boleto</span></span></button></span><br><br>';
            }
        }
    }
