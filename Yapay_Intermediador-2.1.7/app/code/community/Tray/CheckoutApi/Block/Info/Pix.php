...
class Tray_CheckoutApi_Block_Info_Pix extends Mage_Payment_Block_Info
{
    protected function _construct()
    {
        parent::_construct();
        $this->setTemplate('tray/checkoutapi/info/pix.phtml');
    }
    ...
    public function getLinkPayment($order) 
    {
        if ($this->getRequest()->getRouteName() != 'checkout') {
            ...
            if ($method == "traycheckoutapi_pix" && (($_order->getStatus() == Mage_Sales_Model_Order::STATE_PENDING_PAYMENT ) || ($_order->getStatus() == 'pending' ))){
                return '<span><button onclick="window.open(\''.$_order->getPayment()->getData('traycheckout_url_payment').'\');" class="button btn-proceed-checkout btn-checkout" title="PIX" type="button"><span><span>QRCode PIX</span></span></button></span>';
            }
        }
    }
