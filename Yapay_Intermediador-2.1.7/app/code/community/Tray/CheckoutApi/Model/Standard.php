...
class Tray_CheckoutApi_Model_Standard extends Mage_Payment_Model_Method_Abstract
{
        ...
        switch ($data->getMethod()) {
            case 'traycheckoutapi':
                ...
                $info->setCcType($paymentMethod)
                 ...
                 ->setTraycheckoutFingerPrint($data->getTraycheckoutFingerPrint())
                 ->setCcNumber($data->getCcNumber());
                break;
            ...
            case 'traycheckoutapi_pix':
                $paymentMethod = $data->getTraycheckoutPaymentCodePix();
                $info->setCcType($paymentMethod);
                break;                
        }        
    ...
    public function validate()
    {
        ...
        $a = $isOrderVirtual ? $quote->getBillingAddress() : $quote->getShippingAddress();
        ...
        Mage::log('Validate - Street 1 Data: '. $a->getStreet(1), null, 'traycheckout.log');
        if (str_replace(" ","",$a->getStreet(1)) == "") {
            $errorMsg .= "Endereço em branco ou inválido!!\n";
        }
        // Validação do número do endereço.
        Mage::log('Validate - Street 2 Data: '. $a->getStreet(2), null, 'traycheckout.log');
        // if (preg_match('/[a-zA-Z]/',$a->getStreet(2))) {
        if (str_replace(" ","",$a->getStreet(2)) == "") {
            $errorMsg .= "Número do endereço em branco ou inválido!!\n";
        }
        // Verificação se consta o bairro do endereço do cliente
        Mage::log('Validate - Street 4 Data: '. $a->getStreet(4), null, 'traycheckout.log');
        if (str_replace(" ","",$a->getStreet(4)) == "") {
            $errorMsg .= "Bairro em branco ou inválido!!\n";
        }
        // Verificação se consta o cidade do endereço do cliente
        Mage::log('Validate - City Data: '. $a->getCity(), null, 'traycheckout.log');
        if (str_replace(" ","",$a->getCity()) == "") {
            $errorMsg .= "Cidade em branco ou inválido!!\n";
        }
        // Validação do número do endereço.
        Mage::log('Validate - State Data: '. $a->getRegionCode(), null, 'traycheckout.log');
        if (!preg_match('/[A-Z]{2}$/',$a->getRegionCode())) {
            if (!preg_match('/[A-Z]{2}$/',$a->getRegion())) {
    ...
    public function getCheckoutFormFields() 
    {
        ...
        $sArr['transaction[free]']= "MAGENTO_API_v2.1.7";
        ...
        if (strlen($number_taxvat) === 14) {
            $sArr['customer[cnpj]'] = $number_taxvat;
        ...
        $parcelamentoOrder = (($order->getPayment()->getData('traycheckout_split_number') == NULL)|| ($order->getPayment()->getData('traycheckout_split_number') == '0') ? '1' : $order->getPayment()->getData('traycheckout_split_number')); 
        $tcQtdSplit = $tcStandard->getConfigData('tcQtdSplit');

        if ($parcelamentoOrder > $tcQtdSplit) {
            $sArr['transaction[max_split_transaction]'] = $parcelamentoOrder;            
        } else {
            $sArr['transaction[max_split_transaction]'] = $tcQtdSplit;
        }

        $formaPagamento = $order->getPayment()->getData('cc_type');
        
        if (($formaPagamento == '6') OR ($formaPagamento == '7') OR ($formaPagamento == '22') OR ($formaPagamento == '23')) {
            $sArr['transaction[available_payment_methods]'] = '6, 7, 22, 23';
        } else {
            $sArr['transaction[available_payment_methods]'] = '3, 4, 5, 15, 16, 18, 19, 20, 25, 6, 7, 22, 23';
        } if ($formaPagamento == '27') {
            $sArr['transaction[available_payment_methods]'] = '27, 6';
        }
        ...
        if (($order->getPayment()->getData('cc_type') == '3') OR ($order->getPayment()->getData('cc_type') == '4') OR
            ($order->getPayment()->getData('cc_type') == '5') OR ($order->getPayment()->getData('cc_type') == '15') OR
            ($order->getPayment()->getData('cc_type') == '16') OR ($order->getPayment()->getData('cc_type') == '18') OR
            ($order->getPayment()->getData('cc_type') == '19') OR ($order->getPayment()->getData('cc_type') == '20') OR
            ($order->getPayment()->getData('cc_type') == '25')
            ) {
        ...
        $sArr['payment[card_number]'] = str_replace(" ","",$this->decrypt(Mage::getModel('sales/quote_payment')->load($order->getData("quote_id"),"quote_id")->getData("cc_number_enc")));        
    ...
    public function getTrayCheckoutUrl()
    {
         ...
         {
         	return 'https://api.intermediador.sandbox.yapay.com.br';
         } else {
         	return 'https://api.intermediador.yapay.com.br';
         }
    }
    ...
    function updateTransactionTrayCheckout($transactionTc){
        ...
        $quote->getPayment()->setData("traycheckout_payment_method_name", $transactionTc->payment->payment_method_name);
        

        $url_payment = strval($transactionTc->payment->url_payment);
        $quote->getPayment()->setAdditionalInformation('boleto_url', $url_payment);
        ...
        $order->getPayment()->setData("traycheckout_payment_method_name", $transactionTc->payment->payment_method_name);
        $order->getPayment()->setData("boleto_url", $transactionTc->payment->url_payment);
        $url_payment = strval($transactionTc->payment->url_payment);
        $order->getPayment()->setAdditionalInformation('boleto_url', $url_payment);
        ...
        switch ($cod_status){
            ...
            case '24':
                ...
                break;
            case '7':
                    $frase = 'Yapay Intermediador - Cancelado. Pedido cancelado automaticamente.';
                    ...
                    $order->cancel();                
            case '89':                        	
                    $frase = 'Yapay Intermediador - Reprovado. Pedido cancelado automaticamente, o pagamento foi negado.';
