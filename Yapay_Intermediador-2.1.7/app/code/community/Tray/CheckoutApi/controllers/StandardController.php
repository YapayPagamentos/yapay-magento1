...
class Tray_CheckoutApi_StandardController extends Mage_Core_Controller_Front_Action 
{
    ...
    private function getUrlPostCheckoutApi($sandbox)
    {
         if ($sandbox == '1')
         {
            return "https://api.intermediador.sandbox.yapay.com.br/v2/transactions/get_by_token";
         } else {
    ...
    public function successAction() 
    {
        ...
        if($message_response['message'] == "error"){
            ...
            echo "Prefixo: $prefixo1$order->getStatus()$order->getStatus()23 | Pedido: $order_number - ID: ".$transaction['transaction_id'];
            // var_dump($order->getStatus() != 'processing');
            if ($order->getId()) {
    ...
    public function getsplitAction()
    {
        ...
        $tcParcelaMinima = $tcStandard->getConfigData('tcParcelaMinima');
        ...
        for($auxS = 0; $auxS < (int)$tcStandard->getConfigData('tcQtdSplit') && $auxS < sizeof($splittings); $auxS++){
            ...
            if ($splitting->value_split >= floatval($tcParcelaMinima)) {            
                ...
                if ($this->getRequest()->getParam('price', false) <= floatval($tcParcelaMinima))
                    $splitSimulateSplit = array(
                        "1" => "1 x de R$" . (string)$this->getRequest()->getParam('price', false) . " sem juros"
                );
            }
        }
